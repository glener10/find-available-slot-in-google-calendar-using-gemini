...
from src.usecases.scheduler import (
  get_credentials,
  find_events,
  format_events,
  create_event,
...
def main():
  ...
  try:
    ...
    print(
      f"🔎 getting all events between {format_datetime(timeMin)} and {format_datetime(timeMax)}"
    )
    ...
    ai_finding_next_availabe_slot = find_next_available_slot(
      formatted_events, args.start, args.end, args.duration, timeMin
    )
    ...
    user_input = input(
      f"❔ do you want to create the event '{args.name}' at {ai_finding_next_availabe_slot} during {args.duration} minutes? (y/n): "
    )
    if user_input.strip().lower() == "y":
      created_event = create_event(
        service,
        ai_finding_next_availabe_slot,
        args.duration,
        args.name,
        args.invites,
        args.id,
